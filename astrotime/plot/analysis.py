...
class RawDatasetPlot(SignalPlot):
	...
	def get_element_data(self) -> Tuple[np.ndarray,np.ndarray,float,float,str]:
		...
		element: Dict[str,Union[np.ndarray,float]] = self.data_loader.get_element(self.element,unfiltered=True)
...
class ClassificationEvalPlot(SignalPlot):
	...
	def mark_octave_parition(self, octave: int, partition: int ):
		psize = self.nfreq_oct / self.evaluator.oparts
		obase, iprng =  octave*self.nfreq_oct, ( psize*partition, psize*partition+1 )
		pf = [ self.evaluator.f0 * np.power(2.0, obase + ip ) for ip in iprng ]
