...
class WaveletAnalysisLayer(EmbeddingLayer):
	...
	def embed_subbatch(self, ts: torch.Tensor, ys: torch.Tensor, **kwargs ) -> Tensor:
		t0 = time.time()
		self.log.info(f"WaveletAnalysisLayer shapes: ts{list(ts.shape)} ys{list(ys.shape)}")
		slen: int = ys.shape[1]
		...
		dt: Tensor = (ts - tau)
		self.log.info(f" tau{list(tau.shape)} dt{list(dt.shape)}")
		dz: Tensor = omega_ * dt
		weights: Tensor = torch.exp(-self.C * dz ** 2) if (self.cfg.decay_factor > 0.0) else 1.0
		sum_w: Tensor = torch.sum(weights, dim=-1) if (self.cfg.decay_factor > 0.0) else 1.0
		...
		self.log.info(f" ----> p1{list(p1.shape)} p2{list(p2.shape)} mag{list(p2.shape)} phase{list(p2.shape)}")
		self.log.info(f" ----> f0{list(features[0].shape)} f1{list(features[1].shape)} f2{list(features[2].shape)} f3{list(features[3].shape)}")
		embedding: Tensor = torch.stack( features, dim=1)
		self.init_state = False
		self.log.info(f" Completed embedding in {elapsed(t0):.5f} sec: result{list(embedding.shape)}")
		return embedding
