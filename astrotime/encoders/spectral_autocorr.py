...
class SpectralAutocorrelationLayer(OctaveAnalysisLayer):
	...
	def embed(self, ts: torch.Tensor, ys: torch.Tensor, **kwargs ) -> Tensor:
		...
		self.log.info(f"---- SpectralAutocorrelationLayer: sp{list(spectral_projection.shape)}, mean={spectral_projection.mean():.2f} -> sa{sa.shape}, mean={sa.mean():.2f}, std={sa.std()}- ")
		return sa
