...
def wwz(ys: Tensor, ts: Tensor, freq: Tensor, tau: Tensor, device: torch.device, c: float = C0) -> Tuple[Tensor, Tensor, Tuple[Tensor, Tensor, Tensor]]:
    ...
    log.debug(f"wwz-0: nb={nb} nts={nts} nf={nf}")
    log.debug(f" --> devices: ys = {ys.device} ts = {ts.device} freq = {freq.device} tau = {tau.device}")
    ...
    log.debug( f"wwz(nb,nf,nts): ({nb},{nf},{nts}) ys{list(ys.shape)} ts{list(ts.shape)} freq{list(freq.shape)} tau{list(tau.shape)} omega{list(omega.shape)} omega_{list(omega_.shape)} c={c}" )
    dt: Tensor = (ts - tau)
    dz: Tensor = omega_ * dt
    weights: Tensor= torch.exp(-c * dz ** 2)
    sum_w: Tensor = torch.sum(weights, dim=-1)
    log.debug( f"wwz-1: dt{list(dt.shape)} dz{list(dz.shape)} weights{list(weights.shape)} sum_w = {list(sum_w.shape)}")
    log.debug(f" --> devices: dt = {dt.device} dz = {dz.device} weights = {weights.device} sum_w = {sum_w.device}")
    ...
    log.debug( f"wwz-2: sin_basis{list(sin_basis.shape)} one_v{list(one_v.shape)} sin_one{list(sin_one.shape)}  sin_sin{list(sin_sin.shape)} cos_cos{list(cos_cos.shape)}")
    ...
    log.debug(f"wwz-3:  numerator{list(numerator.shape)} denominator{list(denominator.shape)}  time_shift{list(time_shift.shape)} time_shift_{list(time_shift_.shape)}  ")
    log.debug(f" --> devices: numerator = {numerator.device} denominator = {denominator.device} time_shift = {time_shift.device} time_shift_ = {time_shift_.device}")
    ...
    log.debug(f"wwz-4: sin_shift{list(sin_shift.shape)} cos_shift{list(cos_shift.shape)} sin_tau_center{list(sin_tau_center.shape)} cos_tau_center{list(cos_tau_center.shape)}")
    ...
    log.debug(f"wwz-5: ys_cos_shift{list(ys_cos_shift.shape)} cos_shift_one{list(cos_shift_one.shape)} ys_one{list(ys_one.shape)}")

    A: Tensor = 2 * (ys_cos_shift - ys_one * cos_shift_one)
    B: Tensor = 2 * (ys_sin_shift - ys_one * sin_shift_one)

    log.debug(f"wwz-6:  A{list(A.shape)} B{list(B.shape)}")
    a0: Tensor = ys_one
    a1: Tensor = cos_tau_center * A - sin_tau_center * B   # Eq. (S6)
    a2: Tensor = sin_tau_center * A + cos_tau_center * B   # Eq. (S7)

    log.debug(f"wwz-7: a0{list(a0.shape)} a1{list(a1.shape)} a2{list(a2.shape)}")
