...
def UMAP( X: np.ndarray, ncomponents: int ) -> np.ndarray:
	import umap
	reducer = umap.UMAP(n_components=ncomponents)
	Xs = StandardScaler().fit_transform( X )
...
def get_mi_result_plots(args: Namespace, epoch_range_idx, ckpt_type, **kwargs):
	import holoviews as hv
	...
	for train_instance in range(args.ninstances):
		try:
			...
			ctype = f"{args.expt_label}_{nepochs}_{train_instance}"
			if args.reduction_size != 0: ctype += f"_rs{args.reduction_size}"
			results =apply_model(data, args, f"{ctype}.{ckpt_type}" )
